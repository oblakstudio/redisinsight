...
exports.default = {
    ...
    CONNECTION_TIMEOUT: 'The connection has timed out, please check the connection details.',
    ...
    KEY_OR_TIMEOUT_NOT_EXIST: 'Key with this name does not exist or does not have an associated timeout.',
    ...
    COMMAND_NOT_SUPPORTED: (command) => `Redis does not support '${command}' command.`,
    PLUGIN_COMMAND_NOT_SUPPORTED: (command) => `Plugin ERROR: The '${command}' command is not allowed by the RedisInsight Plugins.`,
    PLUGIN_STATE_MAX_SIZE: (size) => `State should be less then ${size} bytes.`,
    ...
    CLI_COMMAND_NOT_SUPPORTED: (command) => `CLI ERROR: The '${command}' command is not supported by the RedisInsight CLI.`,
    ...
    CLUSTER_NODE_NOT_FOUND: (node) => `Node ${node} not exist in OSS Cluster.`,
    REDIS_MODULE_IS_REQUIRED: (module) => `Required ${module} module is not loaded.`,
    APP_SETTINGS_NOT_FOUND: () => 'Could not find application settings.',
    SERVER_INFO_NOT_FOUND: () => 'Could not find server info.',
    INCREASE_MINIMUM_LIMIT: (count) => `Set MAXSEARCHRESULTS to at least ${count}.`,
