"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DatabaseAnalysisModule = void 0;
const common_1 = require("@nestjs/common");
const database_analysis_controller_1 = require("./database-analysis.controller");
const database_analysis_service_1 = require("./database-analysis.service");
const database_analyzer_1 = require("./providers/database-analyzer");
const database_analysis_provider_1 = require("./providers/database-analysis.provider");
const keys_scanner_1 = require("./scanner/keys-scanner");
const key_info_provider_1 = require("./scanner/key-info/key-info.provider");
const recommendation_module_1 = require("../recommendation/recommendation.module");
let DatabaseAnalysisModule = class DatabaseAnalysisModule {
};
DatabaseAnalysisModule = __decorate([
    (0, common_1.Module)({
        imports: [recommendation_module_1.RecommendationModule],
        controllers: [database_analysis_controller_1.DatabaseAnalysisController],
        providers: [
            database_analysis_service_1.DatabaseAnalysisService,
            database_analyzer_1.DatabaseAnalyzer,
            database_analysis_provider_1.DatabaseAnalysisProvider,
            keys_scanner_1.KeysScanner,
            key_info_provider_1.KeyInfoProvider,
        ],
    })
], DatabaseAnalysisModule);
exports.DatabaseAnalysisModule = DatabaseAnalysisModule;
