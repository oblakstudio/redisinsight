...
const homedir = (0, path_1.join)(__dirname, '..');
...
const staticDir = process.env.RI_BUILD_TYPE === 'ELECTRON' && process['resourcesPath']
    ? (0, path_1.join)(process['resourcesPath'], 'static')
    : (0, path_1.join)(__dirname, '..', 'static');
const defaultsDir = process.env.RI_BUILD_TYPE === 'ELECTRON' && process['resourcesPath']
    ? (0, path_1.join)(process['resourcesPath'], 'defaults')
    : (0, path_1.join)(__dirname, '..', 'defaults');
exports.default = {
    dir_path: {
        tmpDir: os.tmpdir(),
        homedir,
    ...
    server: {
        ...
        host: (_a = process.env.RI_APP_HOST) !== null && _a !== void 0 ? _a : '0.0.0.0',
    ...
    redis_clients: {
        ...
        idleThreshold: parseInt(process.env.RI_REDIS_CLIENTS_IDLE_THRESHOLD, 10) || 1000 * 60 * 60,
        syncInterval: parseInt(process.env.RI_REDIS_CLIENTS_SYNC_INTERVAL, 10) || 1000 * 60,
        idleSyncInterval: parseInt(process.env.RI_CLIENTS_IDLE_SYNC_INTERVAL, 10) || 1000 * 60 * 60,
        maxIdleThreshold: parseInt(process.env.RI_CLIENTS_MAX_IDLE_THRESHOLD, 10) || 1000 * 60 * 60,
    ...
    profiler: {
        logFileIdleThreshold: parseInt(process.env.RI_PROFILER_LOG_FILE_IDLE_THRESHOLD, 10) || 1000 * 60,
    },
    analytics: {
    ...
    logger: {
        logLevel: process.env.RI_LOG_LEVEL || 'info',
        stdout: process.env.RI_STDOUT_LOGGER ? process.env.RI_STDOUT_LOGGER === 'true' : false,
        files: process.env.RI_FILES_LOGGER ? process.env.RI_FILES_LOGGER === 'true' : true,
    ...
    connections: {
        timeout: parseInt(process.env.RI_CONNECTIONS_TIMEOUT_DEFAULT, 10) || 30 * 1000,
    },
    redisStack: {
    ...
    features_config: {
        ...
        syncInterval: parseInt(process.env.RI_FEATURES_CONFIG_SYNC_INTERVAL, 10) || 1000 * 60 * 60 * 24,
    },
    cloud: {
        ...
        jobIterationInterval: parseInt(process.env.RI_CLOUD_JOB_ITERATION_INTERVAL, 10) || 10000,
        discoveryTimeout: parseInt(process.env.RI_CLOUD_DISCOVERY_TIMEOUT, 10) || 60 * 1000,
